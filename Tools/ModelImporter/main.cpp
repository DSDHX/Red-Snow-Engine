...
int main(int argc, char* argv[])
{
    ...
    if (scene->HasMaterials())
    {
        ...
        for (uint32_t materialIndex = 0; materialIndex < numMaterials; materialIndex++)
        {
            ...
            aiMaterial->Get(AI_MATKEY_COLOR_EMISSIVE, ambient);
            aiMaterial->Get(AI_MATKEY_COLOR_AMBIENT, emissive);
            ...
            materialData.diffuseMapName = FindTexture(scene, aiMaterial, aiTextureType_DIFFUSE, args, "_diff",
                materialIndex);
            materialData.specMapName = FindTexture(scene, aiMaterial, aiTextureType_SPECULAR, args, "_spec",
                materialIndex);
            materialData.normalMapName = FindTexture(scene, aiMaterial, aiTextureType_NORMALS, args, "_norm",
                materialIndex);
            materialData.bumpMapName = FindTexture(scene, aiMaterial, aiTextureType_HEIGHT, args, "_bump",
                materialIndex);
        }
    }

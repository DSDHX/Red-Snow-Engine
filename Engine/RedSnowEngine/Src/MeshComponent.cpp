...
void MeshComponent::Deserialize(const rapidjson::Value& value)
{
    ...
    ASSERT(value.HasMember("Sphere") || !mMeshModel.meshData.empty(), "MeshComponent: either needs mesh data or has data already");
    ...
    if (value.HasMember("Textures"))
    {
        ...
        SaveUtil::ReadString("DiffuseMap", matData.diffuseMapName, texturesData);
        SaveUtil::ReadString("NormalMap", matData.normalMapName, texturesData);
        SaveUtil::ReadString("SpecMap", matData.specMapName, texturesData);
        SaveUtil::ReadString("BumpMap", matData.bumpMapName, texturesData);
    }
}

...
{
    mInfo = info;
    mNextAvaliableParticleIndex = 0;
    mNextSpawnTime = info.delay;
    mLifeTime = info.lifeTime;

    InitializeParticle(info.maxParticles);
}
...
void ParticleSystem::DebugUI()
{
    ...
    if (ImGui::CollapsingHeader("ParticleSystem"))
    {
        ...
        ImGui::DragIntRange2("PerEmit", &mInfo.particlesPerEmit.min,
            &mInfo.particlesPerEmit.max);
        ImGui::DragFloatRange2("TimeBetweenEmit", &mInfo.timeBetweenEmits.min,
            &mInfo.timeBetweenEmits.max);
        ...
        ImGui::DragFloatRange2("ParticleLifeTime", &mInfo.particleLifeTime.min,
            &mInfo.particleLifeTime.max);
        ...
        ImGui::ColorEdit4("StartColorMin", &mInfo.startColor.min.x);
        ImGui::ColorEdit4("StartColorMax", &mInfo.startColor.max.x);
...
void ParticleSystem::InitializeParticle(uint32_t maxParticles)
{
    mParticles.resize(maxParticles);
    for (uint32_t i = 0; i < maxParticles; i++)
    {
        mParticles[i] = std::make_unique<Patricle>();
...
{
    Patricle* particle = mParticles[mNextAvaliableParticleIndex].get();
    mNextAvaliableParticleIndex = (mNextAvaliableParticleIndex + 1) % mInfo.maxParticles;

...
SoundId SoundEffectManager::Load(const std::filesystem::path& fileName)
{
    ...
    if (success)
    {
        ...
        soundEventPtr->instances = soundEventPtr->effect->CreateInstance();
    }
    return soundId;
}
...
void SoundEffectManager::Clear()
{
    ...
    for (auto& entry : mInventory)
    {
        if (entry.second)
        {
            entry.second->instances->Stop();
            entry.second->instances.reset();
...
void SoundEffectManager::Play(SoundId id, bool loop)
{
    ...
    {
        iter->second->instances->Stop();
        iter->second->instances->Play(loop);
    }
}

...
namespace
{
    std::vector<D3D11_INPUT_ELEMENT_DESC> GetVertexLayout(uint32_t format)
    {
        ...
        if (format & VE_Normal)
        {
            vertexLayout.push_back({ "NORMAL", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 });
        }
        if (format & VE_Tangent)
        {
            vertexLayout.push_back({ "TANGENT", 0, DXGI_FORMAT_R32G32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 });
        }
        if (format & VE_Color)
        {
        ...
        {
            vertexLayout.push_back({ "BLENDINDICES", 0, DXGI_FORMAT_R32G32B32_SINT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0 });
        }

...
void StandardEffecr::Render(const RenderGroup& renderGroup)
{
    ...
    settings.depthBias = mSettingsData.depthBias;
    settings.bumpWeight = mSettingsData.bumpWeight;
    settings.useSkining = mSettingsData.useSkining > 0 && renderGroup.skeleton != nullptr;

    if (settings.useSkining > 0)
    {
...
void StandardEffecr::SetLightCamera(const Camera& camera)
{
    mLightCamera = &camera;
}

void StandardEffecr::SetShadowMap(const Texture& shadowMap)
{
...
void StandardEffecr::DebugUI()
{
    if (ImGui::CollapsingHeader("StandardEffect", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        bool useSkinning = mSettingsData.useSkining > 0;
        if (ImGui::Checkbox("UseSkinning", &useSkinning))
        {
            mSettingsData.useSkining = (useSkinning) ? 1 : 0;
        }
    }
}
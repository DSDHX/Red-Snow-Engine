...
{
    ASSERT(mRenderTargetView == nullptr && mDepthStencilView == nullptr, "RenderTarget: must call terminated");
}

void RenderTarget::Initialize(const std::filesystem::path& fileName)
{
    ASSERT(false, "RenderTarget: dose not support this initialize");
}

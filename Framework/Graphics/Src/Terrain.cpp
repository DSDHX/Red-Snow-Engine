...
void Terrain::Initialize(const std::filesystem::path& fileName, float hightScale)
{
    FILE* file = nullptr;
    ...
    for (uint32_t z = 0; z < rows; ++z)
    {
        for (uint32_t x = 0; x < columns; ++x)
        {
            ...
            const float height = (c / 255.0f) * hightScale;
...
float Terrain::GetHight(const Math::Vector3& position) const
{
    ...
    if (u > v)
    {
        ...
        const Vertex& c = mesh.vertices[tl];
